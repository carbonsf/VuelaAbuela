...
3. GRAMMAR/MEANING — flag ONLY genuine errors, never acceptable variation. ${langName(config.language)} has many equally-correct forms: accept regional vocabulary (e.g. carro / coche / auto / guagua) and ANY valid alternative preposition or phrasing a native speaker would use — for how someone travels, "en carro", "por carro", "en coche", "en auto" are ALL correct; never push one preferred or textbook form over another that is already correct. DO still flag real mistakes: article/adjective–noun gender disagreement (e.g. "una pez" -> "un pez", "el casa" -> "la casa", "un profesora" -> "una profesora"), misspellings, missing or wrong accents (e.g. "Mexico" -> "México", "futbol" -> "fútbol"), and wrong verb conjugations — even when gender is NOT the lesson's grammar target, because a broken answer corrupts the persona it's redistributed into. Do NOT invent errors for correct irregular gender ("el agua", "la mano", "el problema", "el día"). Ignore surrounding punctuation and emphasis, and accept established loanwords. The leniency above is ONLY for vocabulary and phrasing choices (which word, which preposition) — accents, spelling, gender and conjugation are still held to standard. When the only difference from a "preferred" answer is a valid regional or stylistic choice, PASS.
...
- If it answers but has a GENUINE grammar/spelling error -> "correct". ${reveal ? 'Include "answer" with the corrected form.' : 'Do NOT include "answer" — point at the error only so the student derives the fix.'}
- Never "correct" a submission that is already correct: regional or stylistic variants (e.g. "por carro" vs "en carro") are NOT errors -> "pass".
